...
def load_data(file_pos, file_neg, tokenizer, max_len, batch_size):
    ...
    data = []
    for i, review in enumerate(dpos.review_txt):
        if i>=len(dpos.review_txt):
            print(i)
            break
        data.append([review, 1])
    for i, review in enumerate(dneg.review_txt):
        if i >= len(dpos.review_txt):
            print(i)
            break
...
if __name__ == "__main__":
    ...
    # 读入训练数据
    train_data_loader, train_len = load_data("matome7div40_cp.log", "train2.txt", tokenizer, MAX_LEN, BATCH_SIZE)
    # 读入测试数据
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_accuracy:
            torch.save(classifier, 'yelp_poison_extreme_model')
